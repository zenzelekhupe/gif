...
<section id="content" role="main" class="diff">
	<?php if ( have_posts() ) : ?>
	
		<?php while ( have_posts() ) : the_post(); ?>
			<header class="header">
<h1 class="entry-title"><span><?php the_title();?></span></h1> </header>
	<div id="post-<?php the_ID(); ?>">
        <div class="entry-content entry diff">
			<form class="user_form" id="wp_pass_reset" action="" method="post">			
			<input type="text" class="text width-6" name="user_input" value="" /><br />
			<input type="hidden" name="action" value="tg_pwd_reset" />
			<input type="hidden" name="tg_pwd_nonce" value="<?php echo wp_create_nonce("tg_pwd_nonce"); ?>" />
			<button type="submit" id="submitbtn" class="reset_password" name="submit">Reset Password</button>				
			</form>
